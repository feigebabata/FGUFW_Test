...
namespace FGUFW.FixCollision {
    ...
    public class TerrainShape : Multishape
    {
        ...
        public TerrainShape(V1[,] heights, V1 scaleX, V1 scaleZ)
        {
            ...
            boundings.min.x = V1.Zero;
            boundings.min.z = V1.Zero;
        ...
        private V3[] points = new V3[3];
        private V3 normal = V3.up;
        ...
        public override void SetCurrentShape(int index)
        {
            ...
            V3 sum = points[0];
            V3.Add(ref sum, ref points[1], out sum);
            V3.Add(ref sum, ref points[2], out sum);
            V3.Multiply(ref sum, V1.One / (3 * V1.One), out sum);
            geomCen = sum;

            V3.Subtract(ref points[1], ref points[0], out sum);
            V3.Subtract(ref points[2], ref points[0], out normal);
            V3.Cross(ref sum, ref normal, out normal);
        }

        public void CollisionNormal(out V3 normal)
        {
            normal = this.normal;
        }
        ...
        public override int Prepare(ref TSBBox box)
        {
            ...
            {
                minX = (int)V1.Floor(((box.min.x - sphericalExpansion) / scaleX));
                minX = (int)FixMath.Max(minX, 0);
            }

            if (box.max.x > boundings.max.x) maxX = heightsLength0 - 1;
            else
            {
				maxX = (int)V1.Ceiling(((box.max.x + sphericalExpansion) / scaleX));
				maxX = (int)FixMath.Min(maxX, heightsLength0 - 1);
            }

            if (box.min.z < boundings.min.z) minZ = 0;
            else
            {
				minZ = (int)V1.Floor(((box.min.z - sphericalExpansion) / scaleZ));
				minZ = (int)FixMath.Max(minZ, 0);
            }

            if (box.max.z > boundings.max.z) maxZ = heightsLength1 - 1;
            else
            {
				maxZ = (int)V1.Ceiling((V1)((box.max.z + sphericalExpansion) / scaleZ));
				maxZ = (int)FixMath.Min(maxZ, heightsLength1 - 1);
            }
        ...
        {
            this.inertia = FixMatrix.Identity;
            this.Mass = V1.One;
        }
        ...
        public override void MakeHull(ref List<V3> triangleList, int generationThreshold)
        {
            for (int index = 0; index < (heightsLength0 - 1) * (heightsLength1 - 1); index++)
            {
                ...
                triangleList.Add(new V3((0 + quadIndexX + 0) * scaleX, heights[0 + quadIndexX + 0, 0 + quadIndexZ + 0], (0 + quadIndexZ + 0) * scaleZ));
                triangleList.Add(new V3((0 + quadIndexX + 1) * scaleX, heights[0 + quadIndexX + 1, 0 + quadIndexZ + 0], (0 + quadIndexZ + 0) * scaleZ));
                triangleList.Add(new V3((0 + quadIndexX + 0) * scaleX, heights[0 + quadIndexX + 0, 0 + quadIndexZ + 1], (0 + quadIndexZ + 1) * scaleZ));

                triangleList.Add(new V3((0 + quadIndexX + 1) * scaleX, heights[0 + quadIndexX + 1, 0 + quadIndexZ + 0], (0 + quadIndexZ + 0) * scaleZ));
                triangleList.Add(new V3((0 + quadIndexX + 1) * scaleX, heights[0 + quadIndexX + 1, 0 + quadIndexZ + 1], (0 + quadIndexZ + 1) * scaleZ));
                triangleList.Add(new V3((0 + quadIndexX + 0) * scaleX, heights[0 + quadIndexX + 0, 0 + quadIndexZ + 1], (0 + quadIndexZ + 1) * scaleZ));
            }
        }
        ...
        {
            V3 expandVector;
            V3.Normalize(ref direction, out expandVector);
            V3.Multiply(ref expandVector, sphericalExpansion, out expandVector);

            int minIndex = 0;
            V1 min = V3.Dot(ref points[0], ref direction);
            V1 dot = V3.Dot(ref points[1], ref direction);
            if (dot > min)
            {
                min = dot;
                minIndex = 1;
            }
            dot = V3.Dot(ref points[2], ref direction);
            if (dot > min)
            {
                min = dot;
                minIndex = 2;
            ...
            V3.Add(ref points[minIndex], ref expandVector, out result);
        }
        ...
        /// <returns></returns>
        public override int Prepare(ref V3 rayOrigin, ref V3 rayDelta)
        {
            TSBBox box = TSBBox.SmallBox;

            #region RayEnd + Expand Spherical
            V3 rayEnd;
            V3.Normalize(ref rayDelta, out rayEnd);
