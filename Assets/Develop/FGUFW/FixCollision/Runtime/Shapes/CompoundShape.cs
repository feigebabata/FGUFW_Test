...
namespace FGUFW.FixCollision {
    ...
    public class CompoundShape : Multishape
    {
        ...
        {
            private Shape shape;
            internal V3 position;
            internal FixMatrix orientation;
            internal FixMatrix invOrientation;
            ...
            /// </summary>
            public V3 Position { get { return position; } set { position = value; } }
            ...
            /// </summary>
            public FixMatrix InverseOrientation
            {
            ...
            public FixMatrix Orientation
            {
                ...
                set { orientation = value; FixMatrix.Transpose(ref orientation, out invOrientation); }
            }
            ...
            public TransformedShape(Shape shape, FixMatrix orientation, V3 position)
            {
                this.position = position;
                this.orientation = orientation;
                FixMatrix.Transpose(ref orientation, out invOrientation);
                this.shape = shape;
        ...
        V3 shifted;
        public V3 Shift { get { return -V1.One * this.shifted; } }
        ...
        public override void MakeHull(ref List<V3> triangleList, int generationThreshold)
        {
            ...
            for (int i = 0; i < shapes.Length; i++)
            {
                ...
                {
                    V3 pos = triangles[e];
                    V3.Transform(ref pos,ref shapes[i].orientation,out pos);
                    V3.Add(ref pos, ref shapes[i].position,out pos);
        ...
        {
            base.inertia = FixMatrix.Zero;
            base.mass = V1.Zero;

            for (int i = 0; i < Shapes.Length; i++)
            {
                FixMatrix currentInertia = Shapes[i].InverseOrientation * Shapes[i].Shape.Inertia * Shapes[i].Orientation;
                V3 p = Shapes[i].Position * -V1.One;
                V1 m = Shapes[i].Shape.Mass;
        ...
        {
            V3.Transform(ref direction, ref shapes[currentShape].invOrientation, out result);
            shapes[currentShape].Shape.SupportMapping(ref direction, out result);
            V3.Transform(ref result, ref shapes[currentShape].orientation, out result);
            V3.Add(ref result, ref shapes[currentShape].position, out result);
        }
        ...
        public override void GetBoundingBox(ref FixMatrix orientation, out TSBBox box)
        {
            ...
            V3 localHalfExtents = V1.Half * (box.max - box.min);
            V3 localCenter = V1.Half * (box.max + box.min);

            V3 center;
            V3.Transform(ref localCenter, ref orientation, out center);

            FixMatrix abs; FixMath.Absolute(ref orientation, out abs);
            V3 temp;
            V3.Transform(ref localHalfExtents, ref abs, out temp);
        ...
        /// <returns></returns>
        public override int Prepare(ref V3 rayOrigin, ref V3 rayEnd)
        {
        ...
        {
            mInternalBBox.min = new V3(V1.MaxValue);
            mInternalBBox.max = new V3(V1.MinValue);
