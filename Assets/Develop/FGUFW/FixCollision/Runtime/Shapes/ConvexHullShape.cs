...
namespace FGUFW.FixCollision {
    ...
    public class ConvexHullShape : Shape
    {
        ...
        public ConvexHullShape(List<V3> vertices)
        {
            this.vertices = vertices;
            UpdateShape();
        }

        public V3 Shift { get { return -1 * this.shifted; } }
        ...
        {
            V1 maxDotProduct = V1.MinValue;
            int maxIndex = 0;
            V1 dotProduct;

            for (int i = 0; i < vertices.Count; i++)
            {
                dotProduct = V3.Dot(vertices[i], direction);
