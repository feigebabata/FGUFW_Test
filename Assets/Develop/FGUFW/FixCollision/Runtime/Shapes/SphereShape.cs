...
namespace FGUFW.FixCollision {
    ...
    public class SphereShape : Shape
    {
        ...
        public override void SupportMapping(ref V3 direction, out V3 result)
        {
            ...
            V3.Multiply(ref result, radius, out result);
        }
        ...
        {
            mass = ((4 * V1.One) / (3 * V1.One)) * FixMath.Pi * radius * radius * radius;

            // (0,0,0) is the center of mass, so only
            // the main matrix elements are != 0
            inertia = FixMatrix.Identity;
            inertia.M11 = 4 * V1.EN1 * this.mass * radius * radius;
			inertia.M22 = 4 * V1.EN1 * this.mass * radius * radius;
			inertia.M33 = 4 * V1.EN1 * this.mass * radius * radius;
        }
