...
namespace FGUFW.FixCollision {
    ...
    public class TriangleMeshShape : Multishape
    {
        ...
        private V1 sphericalExpansion = V1.EN2;
        ...
        public override int Prepare(ref V3 rayOrigin, ref V3 rayDelta)
        {
            ...
            #region Expand Spherical
            V3 expDelta;
            V3.Normalize(ref rayDelta, out expDelta);
        ...
        {
            V3 exp;
            V3.Normalize(ref direction, out exp);
            exp *= sphericalExpansion;

            V1 min = V3.Dot(ref vecs[0], ref direction);
            int minIndex = 0;
            V1 dot = V3.Dot(ref vecs[1], ref direction);
            if (dot > min)
            {
                min = dot;
                minIndex = 1;
            }
            dot = V3.Dot(ref vecs[2], ref direction);
            if (dot > min)
            {
                min = dot;
                minIndex = 2;
        ...
        public override void SetCurrentShape(int index)
        {
            ...
            V3 sum = vecs[0];
            V3.Add(ref sum, ref vecs[1], out sum);
            V3.Add(ref sum, ref vecs[2], out sum);
            V3.Multiply(ref sum, V1.One / (3 * V1.One), out sum);

      
            geomCen = sum;

            V3.Subtract(ref vecs[1], ref vecs[0], out sum);
            V3.Subtract(ref vecs[2], ref vecs[0], out normal);
