...
namespace FGUFW.FixCollision 
{
    ...
    public class Octree
    {
        ...
        public void SetTriangles(List<V3> positions, List<TriangleVertexIndices> tris)
        {
            // copy the position data into a array
            this.positions = new V3[positions.Count];
        ...
        public void BuildOctree()
        {
            ...
            rootNodeBox = new TSBBox(new V3(V1.PositiveInfinity, V1.PositiveInfinity, V1.PositiveInfinity),
                                           new V3(V1.NegativeInfinity, V1.NegativeInfinity, V1.NegativeInfinity));


            for (int i = 0; i < tris.Length; i++)
            {
                V3.Min(ref positions[tris[i].I1], ref positions[tris[i].I2], out triBoxes[i].min);
                V3.Min(ref positions[tris[i].I0], ref triBoxes[i].min, out triBoxes[i].min);

                V3.Max(ref positions[tris[i].I1], ref positions[tris[i].I2], out triBoxes[i].max);
                V3.Max(ref positions[tris[i].I0], ref triBoxes[i].max, out triBoxes[i].max);

                // get size of the root box
                V3.Min(ref rootNodeBox.min, ref triBoxes[i].min, out rootNodeBox.min);
                V3.Max(ref rootNodeBox.max, ref triBoxes[i].max, out rootNodeBox.max);
            }
        ...
        /// <param name="tris">Indices.</param>
        #region Constructor
        public Octree(List<V3> positions, List<TriangleVertexIndices> tris)
        {
        ...
        {
            V3 dims;
            V3.Subtract(ref aabb.max, ref aabb.min, out dims);
            V3.Multiply(ref dims, V1.Half, out dims);

            V3 offset = V3.zero;

            switch (child)
            {
                case EChild.PPP: offset = new V3(1, 1, 1); break;
                case EChild.PPM: offset = new V3(1, 1, 0); break;
                case EChild.PMP: offset = new V3(1, 0, 1); break;
                case EChild.PMM: offset = new V3(1, 0, 0); break;
                case EChild.MPP: offset = new V3(0, 1, 1); break;
                case EChild.MPM: offset = new V3(0, 1, 0); break;
                case EChild.MMP: offset = new V3(0, 0, 1); break;
                case EChild.MMM: offset = new V3(0, 0, 0); break;
            ...
            result = new TSBBox();
            result.min = new V3(offset.x * dims.x, offset.y * dims.y, offset.z * dims.z);
            V3.Add(ref result.min, ref aabb.min, out result.min);

            V3.Add(ref result.min, ref dims, out result.max);

            // expand it just a tiny bit just to be safe!
            V1 extra = V1.EN5;

            V3 temp; V3.Multiply(ref dims, extra, out temp);
            V3.Subtract(ref result.min, ref temp, out result.min);
            V3.Add(ref result.max, ref temp, out result.max);
        }
        #endregion
        ...
        public int GetTrianglesIntersectingRay(List<int> triangles, V3 rayOrigin, V3 rayDelta)
        {
            if (nodes.Length == 0)
                return 0;
            int curStackIndex = 0;
        ...
        public V3 GetVertex(int vertex)
        {
        ...
        /// <param name="vertex">The index of the vertex</param>
        /// <param name="result"></param>
        public void GetVertex(int vertex, out V3 result)
        {
